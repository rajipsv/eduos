...
**CRM auto-capture:** the same submit also `POST`s to `/api/leads`, which stores the lead server-side (PostgreSQL when `DATABASE_URL` is set, otherwise `server/data/platform-inbound-leads.json`). Log in as **platform owner** (`owner@tutorsala.app`) and open **Demo requests** in the sidebar to view and update stages.
...
The inquiry form opens WhatsApp with a pre-filled message to the Tutorsala sales number (same flow as Pricing on the home page).
